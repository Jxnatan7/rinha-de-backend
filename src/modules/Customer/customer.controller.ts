...
export class CustomersController {
  ...
  @Post(':id/transacoes')
  async createTransaction(@Param('id') id: number, @Body() body: TransactionsRequest) {
    const { valor, tipo, descricao } = body;
    try {
      const transaction = await this.transactionsService.createTransaction(id, valor, tipo, descricao);
      return transaction;
    } catch (error) {
